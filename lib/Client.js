...
function Client(options) {
  ...
  if (parseInt(options.version) == 2) {
    this.options.audience = 'https://' + options.domain + '/api/v2/';
    // TODO: Set this.apiUrl to v2
    // TODO: Implement './api2.js' and expose functions, Object.keys(api2).forEach...
  }
}
...
Client.getAccessToken = function (options, done) {
  ...
  if (options.audience) {
    // This option indicates which version of the API the client intends to use,
    // in other words, the "audience" of the access token.  If this option is
    // not specified, an opaque access token is issued that works with API v1.
    // If this option is set to "https://{tenant}.myauth0.com/api/v2/", a JWT
    // will be issued that works with API v2.
    //
    // Use of an "audience" parameter in the request is defined by the following
    // draft specification:
